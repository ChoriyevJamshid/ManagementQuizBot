...
async def quiz_save_markup():
    texts = await get_texts(('save_button', 'back_text'))
    buttons = (
        f"{texts['save_button']}",
        f"{texts['back_text']}",
    )
...
async def quiz_part_quantity_markup():
    texts = await get_texts(('back_text',))
    buttons = (
        '20',
        '25',
        '30',
