...
async def instruction_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = await get_text("instruction_choice_file_type")
    markup = await inline_kb.instruction_choice_file_type_markup()
...
async def instruction_file_type_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = await get_text(f"instruction_{file_type}", {
        'url': video_url,
    })
